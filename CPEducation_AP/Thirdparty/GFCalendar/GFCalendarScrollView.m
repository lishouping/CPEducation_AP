...
#import "SurveyQuetionTixingModel.h"
...
@property (nonatomic, strong) NSMutableArray *tixingTime;
...
// 当前日期的背景颜色
#define kCalendarBasicColor [UIColor colorWithRed:17.0/255 green:133.0/255 blue:231.0/255 alpha:1]
...
- (void)loadingDate{
    ...
    self.tixingTime = [[NSMutableArray alloc] initWithCapacity:0];
    ...
    [manager POST:postUrl parameters:parameters success:^(AFHTTPRequestOperation *operation, id responseObject) {
        ...
        if ([[responseObject objectForKey:@"code"] isEqualToString:@"1000"]) {
            ...
            {
                SurveyQuetionTixingModel *model = [[SurveyQuetionTixingModel alloc] init];
                model.dateTime = [dic objectForKey:@"date"];
                
                NSLog(@"%@",model);
                
                [self.tixingTime addObject:model];
            }
            [_collectionViewM reloadData];
            [_collectionViewL reloadData];
            [_collectionViewR reloadData];
...
- (__kindof UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (collectionView == _collectionViewL) {
        ...
        if (self.tixingTime.count>0) {
            
            
            
            
            NSInteger *cyear = [[NSString stringWithFormat:@"%ld",monthInfo.year] intValue];
            NSInteger *cmon = [[NSString stringWithFormat:@"%ld",monthInfo.month] intValue];
            NSInteger *cda = [[NSString stringWithFormat:@"%ld", indexPath.row - firstWeekday + 1] intValue];
            
            SurveyQuetionTixingModel *model;
            
            for (int i=0; i<self.tixingTime.count; i++) {
                model = self.tixingTime[i];
                NSArray *arry = [model.dateTime componentsSeparatedByString:@"-"];
                NSInteger *year = [arry[0] intValue];
                NSInteger *mon = [arry[1] intValue];
                NSInteger *da = [arry[2] intValue];
                
                NSLog(@"%@",model.dateTime);
                
                if (year==cyear&&mon==cmon&&da==cda) {
                    cell.tixingView.hidden = NO;
                }
            }
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView {
    ...
    [self.tixingTime removeAllObjects];
