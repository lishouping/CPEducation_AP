//
//  InvestigDetaledImageDownViewController.m
//  CPEducation_AP
//
...
@interface InvestigDetaledImageDownViewController ()
{
...
@implementation InvestigDetaledImageDownViewController
...
- (void)viewDidLoad {
    [super viewDidLoad];
    self.navigationItem.title = @"调查详情";
    self.dateArray = [[NSMutableArray alloc] initWithCapacity:0];
    self.imgDateArray = [[NSMutableArray alloc] initWithCapacity:0];
    [self loadDate];
...
-(void)loadDate{
    ...
    [manager POST:postUrl parameters:parameters success:^(AFHTTPRequestOperation *operation, id responseObject) {
        ...
        if ([[responseObject objectForKey:@"code"] isEqualToString:@"1000"]) {
            ...
            [self.imgDateArray addObjectsFromArray:imgArray];
            ...
            labNoticeTitle.frame = CGRectMake(10, 10, kWidth-10-20, expectSize.height);
            ...
            ischange = 0;
            btnChange = [UIButton new];
            [btnChange setImage:[UIImage imageNamed:@"survey_que"] forState:UIControlStateNormal];
            [btnChange addTarget:self action:@selector(btnChangeClick) forControlEvents:UIControlEventTouchUpInside];
            [headView addSubview:btnChange];
            [btnChange zxp_addConstraints:^(ZXPAutoLayoutMaker *layout) {
                layout.rightSpace(10);
                layout.topSpace(10);
                layout.widthValue(25);
                layout.heightValue(25);
            }];
            
            if (self.imgDateArray.count==0) {
                tableHight = 0;
            }else if (self.imgDateArray.count==1){
                tableHight = 150;
            }else if (self.imgDateArray.count==2){
                tableHight = 300;
            }else if (self.imgDateArray.count==3){
                tableHight = 450;
            }else if (self.imgDateArray.count==4){
                tableHight = 600;
            }else if (self.imgDateArray.count==5){
                tableHight = 750;
            }else if (self.imgDateArray.count==5){
                tableHight = 900;
            }else if (self.imgDateArray.count==5){
            ...
            labNoticeContent.frame = CGRectMake(10, expectSize.height+expectSize1.height+10+10+30, kWidth-10-10, expectSize2.height);
            [headView addSubview:labNoticeContent];
            
            //            UIView *lineView1 = [[UIView alloc] initWithFrame:CGRectMake(10,expectSize.height+expectSize2.height+expectSize1.height+10+10+tableHight+50, kWidth-10-10, 0.5)];
            //            [lineView1 setBackgroundColor:[UIColor colorWithRed:153.0/255.0 green:153.0/255.0 blue:153.0/255.0 alpha:1]];
            //            [headView addSubview:lineView1];
            
            tabViewImg = [[UITableView alloc] initWithFrame:CGRectMake(0, expectSize.height+expectSize1.height+expectSize2.height+60, kWidth, tableHight)];
            tabViewImg.delegate = self;
            tabViewImg.dataSource = self;
            [headView addSubview:tabViewImg];
            
            
            btnLink = [[UIButton alloc] initWithFrame:CGRectMake(kWidth-100, expectSize.height+expectSize2.height+expectSize1.height+10+10+tableHight+60, 100, 15)];
            ...
            headView.frame = CGRectMake(0, 0, kWidth-10-10, expectSize.height+expectSize1.height+10+10+expectSize2.height+60+tableHight+10);
            [self.view addSubview:headView];
            tableViewAnnex.tableHeaderView = headView;
            [tableViewAnnex reloadData];
            [tabViewImg reloadData];
        }
...
//    if (ischange==0) {
//        ischange = 1;
//        [btnChange setImage:[UIImage imageNamed:@"survey_cont"] forState:UIControlStateNormal];
//    }else{
//        ischange = 0;
//        [btnChange setImage:[UIImage imageNamed:@"survey_que"] forState:UIControlStateNormal];
//    }
    InvestigQuestionsViewController *invq = [[InvestigQuestionsViewController alloc] init];
    invq.contentid = self.contentid;
    [self.navigationController pushViewController:invq animated:YES];
}
-(void)addCollect{
...
{
    if ([tableView isEqual:tabViewImg]) {
        return 1;
    }else if ([tableView isEqual:tableViewAnnex]){
        return 1;
    }
    return 1;
}
...
{
    if ([tableView isEqual:tabViewImg]) {
        return self.imgDateArray.count;
    }else if ([tableView isEqual:tableViewAnnex]){
...
{
    if ([tableView isEqual:tabViewImg]) {
        ImageListTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:@"cav"];
        if (cell==nil)
        {
            cell=[[ImageListTableViewCell alloc]initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"avc"];
        }
        cell.selectionStyle=UITableViewCellSelectionStyleNone;
        [cell.pictureImage sd_setImageWithURL:[[self.imgDateArray objectAtIndex:indexPath.row] objectForKey:@"picture"]placeholderImage:[UIImage imageNamed:@"default"]];
        cell.captionLab.text = [[self.imgDateArray objectAtIndex:indexPath.row] objectForKey:@"caption"];
        return cell;
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if ([tableView isEqual:tableViewAnnex]) {
        ...
        NSString *filename = [NSString stringWithFormat:@"/调查-%@%@", title, [[self.dateArray objectAtIndex:indexPath.row] objectForKey:@"filename"]] ;
