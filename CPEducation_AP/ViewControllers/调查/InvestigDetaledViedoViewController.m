//
//  InvestigDetaledViedoViewController.m
//  CPEducation_AP
//
...
@interface InvestigDetaledViedoViewController ()
...
@implementation InvestigDetaledViedoViewController
...
-(void)loadDate{
    ...
    [manager POST:postUrl parameters:parameters success:^(AFHTTPRequestOperation *operation, id responseObject) {
        ...
        if ([[responseObject objectForKey:@"code"] isEqualToString:@"1000"]) {
            ...
            wvVideo = [[UIWebView alloc] initWithFrame:CGRectMake(10, expectSize.height+expectSize1.height+10+10+30, kWidth-10-10, 180)];
            [wvVideo setUserInteractionEnabled:YES];//是否支持交互
            wvVideo.delegate=self;
            [wvVideo setOpaque:NO];//opaque是不透明的意思
            [wvVideo setScalesPageToFit:YES];//自动缩放以适应屏幕
            [headView addSubview:wvVideo];
            NSURL *url = [NSURL URLWithString:video_url];
            [wvVideo loadRequest:[NSURLRequest requestWithURL:url]];
