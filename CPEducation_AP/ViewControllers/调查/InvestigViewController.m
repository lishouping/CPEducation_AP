...
@interface InvestigViewController ()<UITableViewDelegate,UITableViewDataSource,UISearchBarDelegate>{
    UITableView *tabSurvey;
...
@implementation InvestigViewController
- (void)viewWillAppear:(BOOL)animated
{
    ...
    if (myDelegate==nil) {
        ...
        [tabSurvey headerBeginRefreshing];
...
- (void)refreshNotice{
    ...
    if ([myDelegate.pagerefresh isEqualToString:@"1"]) {
        NSLog(@"刷新+1");
        NoticeModelArray *model1 = self.dateArray[pageposition];
        model1.surveystate = @"1";
        NSInteger readnum = [[NSString stringWithFormat:@"%@",model1.surveynumber] integerValue]+1;
        NSString *newreadnum = [NSString stringWithFormat:@"%ld",readnum];
        model1.surveynumber = newreadnum;
        
        NSIndexPath *indexPath1 = [NSIndexPath indexPathForRow:position inSection:0];
        [tabSurvey reloadRowsAtIndexPaths:@[indexPath1] withRowAnimation:UITableViewRowAnimationFade];
        [tabSurvey reloadData];
    }else if ([myDelegate.pagerefresh isEqualToString:@"0"]){
        NSLog(@"刷新-1");
        NoticeModelArray *model1 = self.dateArray[pageposition];
        model1.surveystate = @"0";
        NSInteger readnum = [[NSString stringWithFormat:@"%@",model1.surveynumber] integerValue]-1;
        NSString *newreadnum = [NSString stringWithFormat:@"%ld",readnum];
        model1.surveynumber  = newreadnum;
...
{
    //下拉刷新
    [tabSurvey addHeaderWithTarget:self action:@selector(headerRereshing)];
    //上拉加载
    [tabSurvey addFooterWithTarget:self action:@selector(footerRereshing)];
    //刷新在viewWillAppear
}
...
- (void)createTableView{
    tabSurvey = [[UITableView alloc] initWithFrame:CGRectMake(0, 10, kWidth, kHeight-(10)) style:UITableViewStyleGrouped];
    tabSurvey.delegate = self;
    tabSurvey.dataSource = self;
    [self.view addSubview:tabSurvey];
}
- (void)makeUI{
