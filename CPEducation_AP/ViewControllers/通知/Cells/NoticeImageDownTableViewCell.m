//
//  NoticeImageDownTableViewCell.m
//  CPEducation_AP
//
...
@implementation NoticeImageDownTableViewCell
...
- (void)setModel:(NoticeModelArray *)noticeModel{
    ...
    if ([noticeModel.signstate isEqualToString:@"1"]) {//已签收
        [self.signStateView setBackgroundColor:[UIColor colorWithRed:0.0/255.0 green:137.0/255.0 blue:230.0/255.0 alpha:1]];
        [self.sigState setTextColor:[UIColor whiteColor]];
        [self.sigState setText:@"已签收"];
    }else if ([noticeModel.signstate isEqualToString:@"0"]){
